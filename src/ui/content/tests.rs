...
fn schema_display_lines_highlight_create_sql_snapshot() {
    ...
    let lines = super::schema_display_lines(&app, theme);
    ...
    assert!(sql_lines.iter().any(|line| {
        line.spans.iter().any(|span| {
            span.content.as_ref() == "CREATE" && span.style == theme.syntax_keyword_style()
        })
    }));
    assert_eq!(lines[0].spans.len(), 1);
    assert_eq!(lines[0].spans[0].content.as_ref(), "Table: demo");
    assert_eq!(lines[0].spans[0].style, theme.fg_style());
}
